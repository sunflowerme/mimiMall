...
<html>
    ...
    <script type="text/javascript">
        function fileChange(){//注意：此处不能使用jQuery中的change事件，因此仅触发一次，因此使用标签的：onchange属性
        	...
        	$.ajaxFileUpload({   //异步AJAX的文件上传
				url:"${pageContext.request.contextPath}/prod/ajaxImg.action",
				secureuri: false, //用于当前协议是否遵循安全协议
				fileElementId: 'pimage',   //文件上传的标签属性
				dataType: "json",
				success:function (obj) {  //图片的回显
					//创建一个图片的标签
					var imgObj = $("<img>");
					//给img标签对象追加属性
					imgObj.attr("src","/image_big/" + obj.imgurl);  //完成对于这个图片标签的属性的赋值，函数：attr()
					imgObj.attr("width","100px");
					imgObj.attr("height","100px");
					//将图片追加到imgDiv
					$("#imgDiv").append(imgObj);  //实现图片的回显
				}
			})
           // alert("change");
           //  $.ajaxFileUpload({
				...
				<form action="${pageContext.request.contextPath}/prod/save.action" enctype="multipart/form-data"
					...
					<table>
						...
						<tr>
							...
							<td>
<%--								<select name="typeId">--%>
<%--									<c:forEach items="${typeList}" var="type">  <!--typeList ：就是你在监听器中放的名称-->--%>
<%--										<option value="商品类型的id">商品类别名称</option>--%>
<%--									</c:forEach>--%>
<%--									--%>
<%--								</select>--%>
								<select name="typeId">
